...
## "Notre histoire" étendue à Hestia/Helios (jusqu'ici Charis seulement) — 2026-08-28

Ethan a demandé d'avoir la section "Notre histoire" (`SiteContent.StoryContent`, ajoutée le 2026-08-26 pour Charis) sur les deux autres templates aussi. Aucun changement backend/admin : c'est un champ core déjà éditable depuis "Site internet", seul le rendu public manquait sur Hestia/Helios.

- **`TemplateHestia.tsx`** : nouveau `Band` "Notre histoire" (`Overline` + texte riche) inséré entre le bandeau Établissement et Horaires, avec bascule automatique du fond alterné (`toneFor`) comme les autres sections. Lien de nav "Notre histoire" ajouté en premier (desktop + mobile), même ancre `#histoire` que Charis.
- **`TemplateHelios.tsx`** : nouvelle section (carte blanche `shadow-card`, même style que "Offre du moment") insérée entre l'offre du moment et le bloc de modules, révélée au scroll comme le reste. Même lien de nav ajouté.
- Les deux gardent la même convention que Charis : section absente tant que `storyContent` est vide, titre traduit via la clé déjà existante `nav.story` (fr/en/es, aucune nouvelle clé nécessaire).

**Méthode de vérification notable** : contrairement aux sessions précédentes ("pas d'outil de capture d'écran disponible"), une vérification visuelle a été possible cette fois via Chrome headless piloté en Chrome DevTools Protocol brut (`ws`, déjà présent dans les `node_modules` globaux de la machine, pas de nouvelle dépendance ajoutée au projet) — script Node jetable dans le scratchpad de session, pas commité. Testé en remplissant temporairement `storyContent` sur le tenant historique (Hestia) et sur "Boulangerie Dupont" (basculé temporairement sur Helios/Couchant pour l'occasion, aucun tenant Helios n'existant), capture d'écran des deux, puis les deux tenants remis à l'identique (`storyContent` revidé, template de Boulangerie Dupont restauré sur `hestia`/`argile`) — confirmé par relecture de `/content` et `/template` après coup. `tsc -b` propre.

**Incident pendant le nettoyage** : la commande utilisée pour fermer le Chrome headless de test (`taskkill /IM chrome.exe /T`) cible tous les process `chrome.exe` de la machine, pas seulement celui lancé pour le test — si une fenêtre Chrome normale était ouverte à ce moment, elle a été fermée involontairement. Signalé à Ethan immédiatement. Aucun impact sur les données du projet (le nettoyage des tenants de test avait déjà eu lieu avant cette commande).

Testé : `tsc -b` propre, vérification visuelle des deux templates confirmée par capture d'écran (voir ci-dessus). **Reste à confirmer par Ethan** : simple confirmation visuelle en conditions réelles une fois qu'un tenant Hestia/Helios aura vraiment du texte dans "Notre histoire" (les tests ci-dessus étaient temporaires et déjà retirés).

## Audit responsive mobile des sites publics (Hestia/Helios/Charis) — 2026-08-28 (même jour)

Ethan a demandé un passage sur la version mobile des **sites publics** (jusqu'ici l'audit responsive de juillet avait couvert l'admin en profondeur, et seulement la nav publique Hestia/Helios de façon ciblée — rien depuis sur tout ce qui a été ajouté depuis, notamment Charis). Méthode : Chrome headless/CDP à 390×844, vérification systématique `scrollWidth` vs `clientWidth` (même technique que l'audit admin de juillet) sur home/boutique/fiche produit/panier des 3 templates, plus capture d'écran pour l'inspection visuelle.

**Trouvé — le plus important** : la fiche produit Charis (`ProductPage.tsx`) débordait de 251px en mobile (bouton "Guide des tailles" coupé, slider "Autres produits" qui poussait toute la page). Cause : le composant interne `ProductPageContent` enveloppait son propre contenu dans un second `mx-auto max-w-7xl flex flex-col` alors que le wrapper appelant (`ProductPage`, plus bas dans le même fichier) fournit déjà ce centrage — un `mx-auto` sur un flex item en colonne désactive le `stretch` flexbox (les marges auto priment sur l'étirement, comportement CSS standard mais contre-intuitif) et fait retomber l'élément sur une taille "shrink-to-fit" calculée à partir du contenu le plus large de ses descendants — ici le slider "Autres produits" (cards `shrink-0`), bien plus large que l'écran. Fix : suppression du `mx-auto max-w-7xl` redondant sur `ProductPageContent` (`frontend/src/templates/charis/ProductPage.tsx`), il ne garde que `flex flex-col gap-10`. Vérifié : `scrollWidth` = `clientWidth` après coup, plus rien ne dépasse.
- Les autres pages testées (home/boutique/panier des 3 templates, résultat de commande Charis, blog Hestia) n'avaient pas ce problème — `CataloguePage.tsx` (boutique Hestia/Helios) n'a qu'un seul niveau de `mx-auto max-w-7xl`, pas de doublon.

**Trouvé au passage (pas spécifique au mobile)** : `CatalogueSection.tsx`, `charis/ProductGrid.tsx` et `charis/CataloguePage.tsx` appelaient `res.json()` sans vérifier `res.ok` sur `/catalogue/products` — sur un tenant où le module Catalogue n'est pas autorisé (404), ça levait une erreur console ("Unexpected end of JSON input") à chaque chargement de page, sans impact visible (le composant retombe correctement sur `[]`/liste vide) mais bruyant. Fix : même garde `res.ok ? res.json() : []` déjà utilisée ailleurs dans ces mêmes fichiers (ex. les avis produit).

**Méthode de test** : tenant historique (Hestia) et Atelier Lumen (Charis) testés tels quels ; aucun tenant Helios n'existant, Boulangerie Dupont a été basculée temporairement sur Helios/Couchant (comme pour "Notre histoire" plus haut) puis remise sur Hestia/Argile après coup — confirmé par relecture de `/template`. Chrome headless fermé par PID exact cette fois (`Get-CimInstance Win32_Process` filtré sur `remote-debugging-port=9333`), après un premier nettoyage plus tôt dans la session qui avait utilisé `taskkill /IM chrome.exe /T` par erreur (ferme TOUS les Chrome de la machine, y compris une fenêtre normale d'Ethan si elle était ouverte) — noté en mémoire pour ne pas reproduire.

Testé : `tsc -b` propre. `scrollWidth`/`clientWidth` réconciliés (390=390) sur : Hestia (home, boutique, panier, blog), Helios (home, boutique, panier), Charis (home, boutique, fiche produit, panier, page résultat de commande). Vérification visuelle par capture d'écran sur la plupart de ces pages. **Reste à couvrir** (hors scope de cette passe, à faire si besoin) : pages de contenu personnalisées (module Pages), détail d'article de blog, tous les modules publics un par un (RDV, Galerie, Avis Google, RDV) — testés seulement là où ils apparaissaient déjà sur les pages ci-dessus.

## Retours UX Charis sur capture d'écran mobile réelle — 2026-08-28 (même jour)

Suite immédiate de l'audit mobile ci-dessus : Ethan a testé lui-même sur son téléphone (captures d'écran réelles, pas Chrome headless) et remonté 5 points sur le tenant Atelier Lumen.

**1. Bouton "Panier" flottant positionné trop haut** : `CartButton.tsx` (Charis) appliquait `bottom-24` (96px, pensé pour laisser la place à la barre sticky "Ajouter au panier" de la fiche produit) sur les **3** pages qui réutilisent ce composant (home, boutique, fiche produit) — alors que seule la fiche produit a cette barre. Fix : nouvelle prop `reserveStickyBarSpace` (uniquement passée depuis `ProductPage.tsx`), le bouton revient à `bottom-6` partout ailleurs.

**2. Bouton "Panier" visible même à vide** : `CartButton.tsx` (Charis) et le `CartButton` équivalent d'Hestia/Helios (`modules/catalogue/frontend/CatalogueSection.tsx`) affichaient toujours le bouton, y compris panier vide. Les deux retournent maintenant `null` si `itemCount === 0` — même correctif appliqué aux deux composants pour rester cohérent entre templates, même si seul Charis était visible sur les captures.

**3. Grille à 2 produits mal proportionnée** : les sections "produits d'une collection" ≤ 3 éléments (`ProductGrid.tsx`, `CataloguePage.tsx` — boutique) utilisaient une grille figée `grid-cols-2 sm:grid-cols-2 lg:grid-cols-3` quel que soit le nombre réel de produits : avec exactement 1 ou 2 produits, la grille desktop (`lg:grid-cols-3`) laissait une/deux colonnes vides, rangée visiblement bancale. Nouvelle fonction `staticGridClass(count)` (exportée depuis `ProductGrid.tsx`, réutilisée par `CataloguePage.tsx`) qui plafonne le nombre de colonnes au nombre de produits (1 → `grid-cols-1`, 2 → `grid-cols-2`, 3+ → la grille habituelle). Vérifié en CDP desktop (1440px) : 2 produits occupent maintenant 2 colonnes égales (592px chacune) au lieu de 3 colonnes avec un tiers vide.

**4. Trop d'espace entre la dernière section catalogue et le bloc Contact** : cause réelle, différente de ce qui semblait visible — les sections Galerie/Avis Google/Blog (`TemplateCharis.tsx`, grille `sm:grid-cols-2 gap-8`) se masquent elles-mêmes (`return null`) quand elles n'ont pas de contenu (pas de photo, pas d'avis, pas d'article publié — cas d'Atelier Lumen pour ces trois), mais leur `<div id="...">` conteneur restait dans la grille malgré un enfant vide, consommant quand même son `gap-8` de rangée. Fix : classe `empty:hidden` (variante Tailwind sur `:empty`, ne matche que si React n'a réellement rien rendu dedans) ajoutée aux 6 conteneurs de cette grille (galerie/avis-google/blog/rdv/contact/réseaux-sociaux). Vérifié en CDP : les 3 `<div>` vides passent en `display:none` (0 enfant), l'écart entre la fin du bloc Catalogue et Contact retombe de ~228px à 96px (le `gap-24` intentionnel entre grandes sections, plus rien d'accidentel).

**5. Slider "produits d'une collection" redessiné façon karminecorp.fr** : décidé avec Ethan (`AskUserQuestion`, 3 options proposées) — chaque diapositive devient une mini-fiche produit (grande photo principale fixe + jusqu'à 2 vignettes secondaires toujours visibles en dessous, `images[1]`/`images[2]`), plutôt que la card à hover-swap existante. Nouveau composant `frontend/src/templates/charis/ProductSlideCard.tsx` (garde le même comportement panier — icône hover pour un produit simple, tailles au survol pour un produit à tailles — juste la zone image qui change), `formatPrice`/`CartIcon` exportés depuis `ProductCard.tsx` pour être réutilisés sans dupliquer. `FeaturedSlider` (`ProductGrid.tsx`) utilise ce nouveau composant à la place de `ProductCard` — **décision de portée** : appliqué aux 3 usages du même slider partagé (produits mis en avant sur la home, produits d'une collection, "Nos autres produits" sur la fiche produit) plutôt que de différencier un seul des trois, pour rester visuellement cohérent. La grille statique (≤ 3 produits, point 3 ci-dessus) garde `ProductCard` classique (hover-swap) — le nouveau format est réservé aux sliders, jugé trop haut pour une grille dense.

**Méthode** : Chrome headless/CDP (390×844 puis 1440×900 pour le point 3), panier simulé via `localStorage.setItem('etnof-cart-{id}', ...)` pour tester les deux états du bouton sans passer par un vrai paiement. Chrome fermé par PID exact à chaque fois (`Get-CimInstance Win32_Process` filtré sur la ligne de commande) — aucun incident cette fois.

Testé : `tsc -b` propre après chaque étape. Vérifié en CDP : bouton panier à `bottom-6`/`24px` sur home et boutique, `bottom-24`/`96px` sur la fiche produit ; bouton absent panier vide, présent avec le bon compte panier rempli ; grille 2 produits en 2 colonnes égales sur desktop ; 3 `<div>` de modules vides passées en `display:none`, écart Catalogue→Contact ramené à 96px ; nouvelle card slider vérifiée visuellement (capture d'écran) sur "Pièces fortes" — grande photo + 2 vignettes + nom/prix, rendu conforme à la référence. **Reste à confirmer par Ethan** : les 5 points sur son téléphone réel (tout a été vérifié par CDP/simulation, pas par un vrai appareil pour cette session).

## 4 retours supplémentaires sur les mêmes captures d'écran réelles — 2026-08-28 (même jour)

Suite immédiate — Ethan a testé les 5 correctifs ci-dessus sur son téléphone et remonté 4 nouveaux points, toujours sur Charis :

**1. Header pas fixe au scroll en mobile** : `SiteChrome.tsx` — `<nav>` passe en `sticky top-0 z-30 md:static` (desktop inchangé, comme demandé). Fond changé au passage : l'ancien fond translucide (`${ink}06`, ~2% d'opacité, pensé pour une nav non-collante qui ne recouvre jamais rien) laissait transparaître le contenu qui défile derrière une fois collée — remplacé par `background` (couleur pleine de la page), imperceptible en desktop mais indispensable en sticky.

**2. Rendu "pas ouf" avec seulement 2 photos** : `ProductSlideCard.tsx` (ajouté juste avant) affichait la vignette secondaire seule étirée en pleine largeur quand le produit n'a que 2 photos au total — trop grande, déséquilibré. Fix : la rangée de vignettes ne s'affiche plus que si les **2** sont disponibles (3 photos ou plus au total) ; avec seulement 2 photos, seule la photo principale s'affiche, comme un produit à 1 photo.

**3. Chaque diapositive du slider ne prend pas tout l'écran (on voit le début de la suivante)** : le "peek" du produit suivant (`w-[85%]` en mobile, `FeaturedSlider` dans `ProductGrid.tsx`) était en fait volontaire à l'origine (indice visuel qu'il y a plus à faire défiler) mais jugé "moche" par Ethan. Passé à `w-full` en mobile (un seul produit visible à la fois, on swipe pour voir le suivant) — `sm:`/`lg:` inchangés (toujours calculés en fraction pour garantir 2/3 tuiles visibles à ces tailles).

**4. Rendu "moche" en dessous de 3 produits sous "Nos collections phare"** : la grille statique (≤ `SLIDER_THRESHOLD`) utilisée pour les petites sections ne convient plus en mobile depuis que les cards du slider sont devenues plus hautes (photo + vignettes) — deux cards hautes serrées côte à côte sur un écran étroit rendait mal. Fix : double rendu conditionnel par breakpoint (`sm:hidden` / `hidden sm:grid`) sur les deux endroits qui avaient ce choix grille-ou-slider (produits "mis en avant" et produits d'une collection, tous deux dans `ProductGrid.tsx`) — mobile passe toujours par `FeaturedSlider` quel que soit le nombre de produits (1, 2 ou plus), desktop garde la grille pleine largeur en dessous du seuil. Même mécanisme déjà utilisé ailleurs dans le projet pour un affichage différent par breakpoint (ex. sidebar/drawer admin) — deux rendus existent dans le DOM, un seul visible à la fois selon la largeur d'écran.

**Portée assumée** : les points 3 et 4 ne touchent que `ProductGrid.tsx` (home) — la grille de la page boutique complète (`CataloguePage.tsx`) n'a pas cette dichotomie grille/slider (toujours une grille statique, jamais mentionnée comme problème) et n'a pas été touchée.

Testé : `tsc -b` propre. Vérifié en CDP (390×844, scroll simulé à 1500px) : nav `position:sticky`, `top:0` confirmé après scroll, capture d'écran confirmant un fond plein sans chevauchement visuel ; carte "Robe Midi Sable" (2 photos) n'affiche plus de vignette solitaire ; diapositive pleine largeur sans aperçu du produit suivant ; "Essentiels du quotidien" (2 produits) passe bien par le slider en mobile au lieu de la grille. **Reste à confirmer par Ethan** : les 4 points sur son téléphone réel (vérifiés par CDP cette session, pas par un vrai appareil).

## 2 derniers retours sur les mêmes captures : flèches du slider + "Notre histoire" tronquée — 2026-08-28 (même jour)

**1. Flèches de slider absentes en dessous de 4 produits** : `FeaturedSlider` (`ProductGrid.tsx`) ne montrait les flèches `‹`/`›` que si `products.length > 3` — reliquat de l'époque où seuls les gros sliders (4+ produits) passaient par ce composant. Depuis le forçage du slider en mobile même à 1-2 produits (point 4 ci-dessus), les petites collections n'avaient plus aucun moyen de naviguer visuellement. Seuil abaissé à `products.length > 1`.

**2. "Notre histoire" sans troncature, texte long = pavé énorme** : ajout d'un `line-clamp-4` + bouton "Voir plus"/"Voir moins", même patron déjà en place pour les avis Google (`ReviewCard` dans `AvisGoogleSection.tsx` — seuil de longueur de texte brut plutôt qu'une vraie mesure DOM, ~220 caractères pour un avis). Appliqué aux **3 templates** (pas seulement Charis, montré sur les captures) puisque "Notre histoire" existe désormais partout depuis ce matin : `StorySection.tsx` (Charis), `TemplateHestia.tsx`, `TemplateHelios.tsx` — chacun avec son propre état `expanded`/seuil `LONG_STORY_THRESHOLD` (320 caractères, un peu plus permissif qu'un avis puisque ce texte est plus habituellement long) et sa fonction `stripHtml` locale (mesure sur le texte brut, balises retirées — le contenu est du HTML riche via `RichTextEditor`). Nouvelles clés de traduction `story.showMore`/`story.showLess` (fr/en/es), à côté de `nav.story` déjà existante.

Testé : `tsc -b` propre. Vérifié en CDP : flèches visibles sur "Essentiels du quotidien" (2 produits) ; texte "Notre histoire" d'Atelier Lumen (texte 3 paragraphes) tronqué à 4 lignes avec "Voir plus" affiché, clic simulé → passe à "Voir moins" et le clamp disparaît (texte complet affiché). **Reste à confirmer par Ethan** : les 2 points sur son téléphone réel, et un coup d'œil sur Hestia/Helios (pas de vrai contenu long testé sur ces deux templates cette session, seulement Charis).

## Retouches mid-session (troncature à 10 lignes, badge collection, fiche produit) — 2026-08-28 (même jour)

Trois derniers retours d'Ethan sur la même série de captures, traités à la suite (les deux premiers reçus en cours de travail sur le troisième) :

**1. Troncature "Notre histoire" trop courte** : 4 lignes ne laissaient voir presque rien. Seuil de troncature relevé à ~10 lignes sur les 3 templates (`line-clamp-4` → `line-clamp-[10]`, valeur arbitraire Tailwind — les paliers standards s'arrêtent à 6) et `LONG_STORY_THRESHOLD` 320 → 800 caractères en conséquence (`StorySection.tsx`, `TemplateHestia.tsx`, `TemplateHelios.tsx`).

**2. Badge collection redondant sur les sliders de la home** : sur "Nos collections phare" (`ProductGrid.tsx`), chaque card affichait un badge répétant le nom de la collection déjà annoncé par le `<h3>` juste au-dessus (ex. "Pièces fortes" en titre et en badge sur chaque card en dessous) — retiré uniquement à cet endroit (`collectionsById={{}}` sur les deux `FeaturedSlider`, mobile et desktop ; prop `collectionName` retirée de la grille statique `ProductCard`). Volontairement laissé en place sur le slider "mis en avant" (en haut de la home) et sur "Nos autres produits" (fiche produit) : ces deux-là mélangent plusieurs collections, le badge y reste informatif.

**3. Trois retouches sur la fiche produit Charis (mobile)** :
- Écart fil d'Ariane → galerie trop grand : `gap-10` → `gap-6 sm:gap-10` sur le conteneur racine de `ProductPageContent`.
- Description produit trop peu lisible : couleur `${palette.ink}99` (60% d'opacité) → `${palette.ink}CC` (80%).
- Section Newsletter ajoutée tout en bas de la page (après "Nos autres produits", avant le footer) — absente jusqu'ici de cette page, demandée par Ethan à l'image de karminecorp.fr. Nécessite de faire descendre `modules` (déjà chargé par `ProductPage`, jamais transmis à `ProductPageContent` jusqu'ici) via une nouvelle prop.

Testé : `tsc -b` propre. Vérifié en CDP : "Notre histoire" affiche nettement plus de texte avant troncature ; carte "Pièces fortes" sans badge sur le slider de collection (comparée au slider "mis en avant" qui le garde) ; capture d'écran de la fiche produit confirmant l'écart réduit, la description plus foncée, et le bloc Newsletter visible juste avant le footer. **Reste à confirmer par Ethan** : ces 3 derniers points sur son téléphone réel.

**Suite immédiate — même badge retiré sur la page boutique** : `CataloguePage.tsx` avait le même défaut que "Nos collections phare" sur la home — chaque section groupée par collection (`sections.map`, titre `<h2>` + grille) répétait le badge sur chaque card alors que le titre de section (ou, filtre actif, le chip sélectionné) l'annonce déjà. Contrairement à la home, **toutes** les sections de cette page sont dans ce cas (pas d'équivalent "mis en avant"/"produits liés" mélangeant les collections) — `collectionName` retiré sans condition de l'unique usage de `ProductCard` dans ce fichier. `collectionsById` (calculé pour cet unique usage) devenu mort a été retiré au passage (`noUnusedLocals` l'aurait fait échouer au build sinon).

Testé : `tsc -b` propre. Vérifié en CDP (`/boutique`, vue "Tout") : aucune des 4 cards (2 par collection) n'affiche plus de badge, `scrollWidth`/`clientWidth` réconciliés, aucune erreur console.

## Filtres boutique : compteur de produits + repli en `<select>` au-delà de 5 collections — 2026-08-28 (même jour)

Deux demandes d'Ethan sur les pastilles de filtre de `/boutique` (`CataloguePage.tsx`) : afficher le nombre de produits par filtre, et une question sur le risque d'encombrement visuel si beaucoup de collections existent.

- **Compteur** : "Tout (N)" et "{collection} (N)" — `N` calculé côté client (`products.filter(...)`, pas de nouvel appel API), y compris dans le nouveau mode `<select>` ci-dessous.
- **Repli en `<select>` natif au-delà de `CHIP_THRESHOLD = 5` collections** : décision prise sans repasser par Ethan (question ouverte, réponse tranchée directement) — en dessous du seuil, les pastilles restent (plus visibles, dans l'esprit du reste du site) ; au-delà, un `<select>` remplace la rangée de pastilles pour éviter un mur de boutons sur plusieurs lignes. **Décision technique notable** : le composant `Select.tsx` déjà utilisé partout en admin n'a pas été réutilisé ici — ses couleurs (`bg-brand-mid`, `text-navy`, etc.) sont la charte fixe de l'admin, pas la palette dynamique du tenant (`palette.accent/ink`) que suivent tous les composants publics Charis ; l'aurait réutilisé tel quel aurait affiché les mauvaises couleurs sur le site public. Un `<select>` natif est utilisé à la place (stylé minimalement via la palette pour son état fermé, menu déroulant natif du navigateur pour le reste) — jugé suffisant pour ce cas d'usage edge-case (beaucoup de collections), contrairement à l'admin où un `<select>` natif avait déjà posé problème (voir session du 2026-08-26, "menu Statut en gris système").

Testé (`tsc -b` propre) en CDP, en créant temporairement 4 collections vides sur Atelier Lumen (6 au total, au-delà du seuil) puis en les supprimant après vérification (`POST`/`DELETE /admin/catalogue/collections`, tenant remis à l'identique — confirmé par relecture de `/catalogue/collections`, 2 collections restantes) : mode pastilles avec compteurs corrects ("Tout (6)", "Essentiels du quotidien (2)", "Pièces fortes (4)") en dessous du seuil ; bascule automatique en `<select>` au-dessus, options et compteurs vérifiés via le DOM (`"Tout (6)"`, `"Essentiels du quotidien (2)"`, `"Pièces fortes (4)"`, 4× `"Test Collection X (0)"`). **Reste à confirmer par Ethan** : rendu du `<select>` sur son téléphone réel (jamais testé au-delà de 2 collections en vrai).

## Panier déplacé dans le header, réordonnancement home, espacement Contact/Réseaux/Newsletter — 2026-08-28 (même jour)

Trois derniers retours d'Ethan, toujours sur la même série de captures Atelier Lumen mobile :

**1. Bouton "Panier" flottant qui "vole" sur la fiche produit** : le bouton flottant (`CartButton.tsx`, `position:fixed`) se superposait aux miniatures/texte en défilant, en plus de coexister avec la barre sticky "Ajouter au panier" déjà présente sur cette page — deux éléments fixes redondants. Remplacé par une icône panier permanente dans le header, façon karminecorp.fr : nouvelle `CartIconLink` dans `SiteChrome.tsx` (icône sac + pastille du nombre d'articles), visible sur les 3 pages Charis (home, boutique, fiche produit) puisque `SiteChrome` leur est commun, gate sur `modules?.catalogue?.enabled`.
- **Changement de structure nécessaire** : l'icône du header doit lire `useCart()`, mais `SiteChrome` (qui contient la nav) est monté **au-dessus** du `CartProvider` que chacune des 3 pages créait jusqu'ici pour son propre usage (`ProductGrid.tsx`, `CataloguePage.tsx`, `ProductPage.tsx`) — la nav n'avait donc jamais accès au panier. `CartProvider` déplacé dans `SiteChrome.tsx` lui-même (englobe nav + contenu + footer) ; les 3 fichiers appelants ont perdu leur `CartProvider` imbriqué (un panier dupliqué et désynchronisé du compteur du header sinon) et leur usage de `CartButton`.
- `CartButton.tsx` supprimé (plus aucun appelant). La barre sticky mobile "Ajouter au panier" de la fiche produit (élément différent, ancrée en bas sans chevaucher le contenu) n'a pas été touchée — pas concernée par la plainte.

**2. Catalogue trop bas sur la home** : conséquence directe de la troncature "Notre histoire" passée à ~10 lignes plus tôt dans la session — le bloc, désormais bien plus haut, retardait l'arrivée sur les produits. `TemplateCharis.tsx` : section Catalogue remontée **avant** "Notre histoire" (jusqu'ici l'inverse, décision de la session du 2026-08-25) — les acheteurs arrivent directement sur les produits, le contenu de marque vient juste après.

**3. Écarts Contact→Réseaux sociaux→Newsletter incohérents** : Contact et Réseaux sociaux partagent une grille (`gap-8` = 32px), mais Newsletter était rendue **hors** de cette grille (son propre `<Reveal>`/`<Suspense>`, dans le conteneur extérieur à `gap-24` = 96px) — écart visiblement différent du précédent. Newsletter déplacée dans la même grille, toujours en dernier et toujours `sm:col-span-2` (pleine largeur, jamais coincée dans une colonne partagée avec les réseaux sociaux — l'exigence d'origine du 2026-08-07 reste respectée, seul le conteneur change).

Testé : `tsc -b` propre. Vérifié en CDP : bouton flottant absent (`document.querySelector('a[href*="/panier"].fixed')` → false), icône panier présente dans le header avec la pastille "1" (capture d'écran à l'appui) ; home : Catalogue apparaît juste après le hero, "Notre histoire" après "Nos collections phare" ; écarts Contact→Réseaux sociaux et Réseaux sociaux→Newsletter mesurés à 32px chacun (avant : 32px puis 96px). **Reste à confirmer par Ethan** : ces 3 points sur son téléphone réel, en particulier que l'icône panier reste pratique d'accès (remplace un bouton toujours visible par une icône qu'il faut remarquer dans le header).

## Page panier : ligne d'article restructurée + champ téléphone élargi — 2026-08-28 (même jour)

Trois derniers retours d'Ethan, sur des captures de `/panier` (page partagée par les 3 templates, pas Charis seul) :

**1 et 2. Produit pas assez visible + boutons quantité/suppression collés à la description** : cause commune — la ligne d'article (`CartPage.tsx`) tenait tout sur une seule rangée `flex items-center justify-between` (vignette 56px + nom + prix d'un côté, `−`/quantité/`+`/"Retirer" de l'autre), ce qui écrasait le nom du produit contre les contrôles à 390px. Fix : `flex-col` par défaut (image+infos, puis quantité/suppression sur leur propre ligne en dessous, pleine largeur) → `sm:flex-row` dès qu'il y a la place. Vignette agrandie au passage (`h-14 w-14` → `h-20 w-20`, 56px → 80px) pour répondre au "on ne voit pas assez le produit".

**3. Champ téléphone trop petit** : `PhoneInput.tsx` (`frontend/src/components/admin/PhoneInput.tsx`, partagé avec l'admin — Établissement, voir session du 2026-08-26) réserve une largeur fixe `w-48` (192px) au sélecteur de pays, ne laissant presque rien au champ numéro sur un écran étroit (~94px mesurés, placeholder "Téléphone" tronqué en "Téléphon…"). Passé à `w-32 sm:w-48` : 128px en mobile (le texte du sélecteur tronque déjà proprement, voir `Select.tsx`), 192px inchangé à partir de `sm` — l'admin (plutôt desktop) n'est pas affecté.

Testé : `tsc -b` propre. Vérifié en CDP (`/panier`, Atelier Lumen, un article en panier) : ligne d'article en deux temps (nom "Trench Long Beige" sur une seule ligne, contrôles quantité/suppression bien en dessous, pleine largeur) ; champ téléphone mesuré à 158px (contre ~94px avant), placeholder "Téléphone" affiché en entier. **Reste à confirmer par Ethan** : ces 3 points sur son téléphone réel.

## Notification email au tenant (nouvelle commande / nouveau message de contact) — 2026-08-28 (même jour)

Après le point sur les retouches mobiles Charis, Ethan a validé une liste d'idées de fonctionnalités proposées côté admin/expérience client (recherche produit, liste de souhaits, compte client, export commandes, multi-comptes admin, notification tenant) — celle-ci traitée en premier (meilleur rapport effort/valeur : jusqu'ici rien ne prévenait le commerçant d'une nouvelle commande ou d'un nouveau message, il fallait penser à retourner voir son admin).

- **`BrevoEmailService.cs`** : deux nouvelles méthodes, même patron que `SendOrderConfirmationEmailAsync`/`SendSupportRequestAsync` déjà en place — `SendNewOrderNotificationAsync` (déclenchée juste après l'email de confirmation client, dans le webhook Stripe) et `SendNewContactMessageNotificationAsync` (déclenchée à la soumission du formulaire de contact, `ContactModule.cs`). Les deux sont best-effort (try/catch, jamais laissées faire échouer l'enregistrement de la commande/du message, même logique que l'existant).
- **Adresse de notification** : `SiteContent.ManagerEmail` (contact interne, jamais affiché publiquement — ajouté en 2026-08-27) si renseigné, sinon repli sur `SiteContent.Email` (public) — mieux vaut prévenir quelque part que nulle part. Si aucun des deux n'est renseigné, aucun envoi (comportement silencieux, pas de bascule à activer/désactiver — reste simple, cf. règle 7 de `CLAUDE.md`).
- Les deux emails incluent un bouton vers la page admin concernée (`/admin/{clientSiteId}/orders` ou `/admin/{clientSiteId}/messages`, URL construite depuis `Cors:AllowedOrigin` comme déjà fait pour les factures) — absent des emails **clients** existants (pas de page de suivi publique, décision déjà prise).
- Notification de message de contact avec `ReplyTo` pointant vers l'expéditeur du message — le commerçant peut répondre directement depuis sa boîte mail.

Testé : `dotnet build -c Release` propre (0 erreur), backend redémarré pour charger le nouveau code. Message de contact test soumis sur le tenant historique après avoir temporairement renseigné `ManagerEmail` avec la boîte de test déjà utilisée par le projet (`etnofweb@gmail.com`, voir session "Bulle de contact support" du 2026-07-31) : `POST /contact` → `201`, log backend confirmant l'appel à l'API Brevo → `201` (email accepté et mis en file). Tenant remis à l'identique (`managerEmail` revidé, message de test supprimé) après vérification. **Chemin commande non testé de bout en bout** (nécessiterait un vrai paiement Stripe test) — code revue seule : réutilise exactement le même `BrevoEmailRequest`/client HTTP que la confirmation client, déjà prouvée fonctionnelle par ce test.

**Reste à faire, pas dans cette session** (proposées par Ethan, à prioriser) : recherche produit sur le site public, liste de souhaits, compte client (historique de commandes), export CSV des commandes/clients, multi-comptes admin par tenant.

## Export CSV commandes/clients — 2026-08-28 (même jour)

Premier point de la liste ci-dessus traité (le plus simple, aucun risque d'architecture) : bouton "Exporter en CSV" sur les pages admin Commandes et Clients.

- Nouveau `frontend/src/utils/csv.ts` (`downloadCsv`, réutilisé par les deux pages) : construction du CSV et déclenchement du téléchargement côté client, pas de nouvel endpoint (les deux pages ont déjà la liste complète en mémoire). Séparateur point-virgule + BOM UTF-8 (convention Excel FR — sinon les accents s'affichent mal à l'ouverture).
- `OrdersSection.tsx` : exporte la vue **actuellement filtrée** (recherche + filtre de statut déjà appliqués), pas toutes les commandes sans distinction — cohérent avec ce que le commerçant voit à l'écran.
- `CustomersSection.tsx` : bouton à côté de "+ Ajouter un client", désactivé si la liste est vide.

Testé (CDP/Chrome headless, en pilotant un vrai clic sur le bouton avec `Browser.setDownloadBehavior`) sur Atelier Lumen (2 commandes, 2 clients réels) : les deux CSV téléchargés et relus — BOM UTF-8 présent, séparateur `;`, accents corrects (`Téléphone`, `Total dépensé`), lignes conformes aux tableaux affichés. **Incident mineur** : `Browser.setDownloadBehavior` avec un dossier personnalisé n'a pas été respecté par ce Chrome headless — les 3 fichiers de test sont partis dans le vrai dossier Téléchargements d'Ethan, supprimés immédiatement après vérification du contenu. `tsc -b` propre.

## Recherche produit sur le site public — 2026-08-28 (même jour)

Deuxième point de la liste de fonctionnalités traité : champ de recherche sur la page boutique, purement côté client (les deux pages boutique ont déjà la liste complète des produits en mémoire, pas de nouvel endpoint) — filtre par nom de produit, insensible à la casse.

- **`modules/catalogue/frontend/CatalogueSection.tsx`** (boutique simple Hestia/Helios, aussi utilisée en teaser limité sur la home) : champ affiché seulement quand `!limit` (jamais sur le teaser home). `visibleProducts` recalculé à partir des produits filtrés par la recherche avant application du `limit`/tri "mis en avant".
- **`frontend/src/templates/charis/CataloguePage.tsx`** (boutique riche Charis, chips/`<select>` de collection) : recherche appliquée **avant** le regroupement par collection — un chip garde toujours le total réel de sa collection (`collectionCounts` reste basé sur tous les produits, pas la recherche en cours), seuls les produits affichés dans chaque section sont filtrés.
- Message dédié si la recherche ne donne aucun résultat (`catalogue.noSearchResults`, distinct de `catalogue.emptyCollection`) — nouvelles clés `catalogue.searchPlaceholder`/`catalogue.noSearchResults` (fr/en/es).

Testé (CDP) : sur Charis (Atelier Lumen), recherche "trench" → 1 résultat exact ; recherche sans correspondance → message "Aucun produit ne correspond à cette recherche." affiché. Sur Hestia (tenant historique), recherche "bougie" → les 3 produits candle correctement isolés du reste du catalogue. Champ absent du teaser home (`limit` fourni) sur les deux templates, confirmé par requête DOM. `scrollWidth`/`clientWidth` réconciliés, aucune erreur console. `tsc -b` propre.

## Retouches desktop sur la recherche + taille des cards produit — 2026-08-28 (même jour)

Deux retours d'Ethan sur des captures **desktop** (première fois cette session — tout le reste était vérifié en mobile) :

**1. Champ de recherche mal placé** : affiché au-dessus des pastilles de collection plutôt qu'à côté du lien "← Retour au site" (`frontend/src/templates/charis/CataloguePage.tsx`). Nécessite de lever l'état `search` du composant `CatalogueContent` vers son parent `CharisCataloguePage` (le lien "Retour au site" vit dans le parent, pas dans `CatalogueContent`) — passé en prop, plus d'état interne dupliqué. Les deux tiennent maintenant sur la même ligne (`flex flex-wrap justify-between`, repasse naturellement l'un sous l'autre en mobile faute de place).

**2. Cards produit démesurées en desktop avec 1-2 produits dans une collection** : `staticGridClass` (ajouté ce matin pour corriger l'inverse — un "trou" de colonne vide) plafonnait le **nombre de colonnes** au nombre de produits (`grid-cols-2` fixe pour 2 produits) — sur un conteneur `max-w-7xl` en desktop, chaque card grossissait alors à ~550px de large au lieu de la taille normale (~384px, celle d'une collection à 3+ produits). Repensé : à partir de `sm`, la grille bascule sur `[grid-template-columns:repeat(auto-fill,24rem)]` (24rem = taille d'une colonne de `lg:grid-cols-3` sur ce conteneur, calcul détaillé en commentaire) — le navigateur calcule lui-même combien de colonnes de 24rem tiennent dans la largeur réelle, sans jamais dépasser cette taille par card ni étirer les cards pour combler l'espace restant (qui reste simplement vide à droite, comme une grille normale à ligne incomplète). Mobile inchangé (garde le plafonnage par nombre de produits ajouté ce matin, pas concerné par le problème desktop) — et de toute façon la home mobile ne rend même plus cette grille du tout depuis le forçage du slider (`hidden sm:grid`).
- Portée : `staticGridClass` est partagée par `ProductGrid.tsx` (home) et `charis/CataloguePage.tsx` (boutique) — un seul endroit à corriger a suffi pour les deux pages, comme demandé par Ethan ("de même sur la page boutique").

Testé (CDP, viewport desktop 1280px) : cards de "Essentiels du quotidien" (2 produits) et "Pièces fortes" (4 produits) mesurées à exactement 384px chacune (identique), capture d'écran confirmant un rendu cohérent entre les deux sections ; champ de recherche visuellement aligné avec "← Retour au site" sur la même ligne. Re-vérifié en mobile (390px) : aucune régression, slider home toujours actif, grille boutique toujours à 2 colonnes raisonnables. `tsc -b` propre, `scrollWidth`/`clientWidth` réconciliés, aucune erreur console.

## Header Charis collant étendu au desktop — 2026-08-28 (même jour)

Le `sticky` posé ce matin uniquement en dessous de `md` (desktop explicitement laissé statique à ce moment-là) étendu à tous les breakpoints — cohérent maintenant que l'icône panier vit dans le header (accessible en permanence en scrollant, plus seulement en mobile). `SiteChrome.tsx` : classe `md:static` retirée du `<nav>`, le fond déjà opaque posé ce matin pour le mode sticky mobile n'a pas eu besoin d'être retouché.

Testé (CDP, 1280px, scroll simulé à 1500px) : nav confirmée `position:sticky`/`top:0` après scroll en desktop. `tsc -b` propre.

## Nouveau module "Compte client" (connexion par lien email, historique de commandes) — 2026-08-28 (même jour)

Premier des "gros chantiers" évoqués avec Ethan cette session. Contrairement au reste de la session (retouches ponctuelles), celui-ci a été précédé d'une vraie discussion de cadrage (pas un POC bâclé, demande explicite d'Ethan) : périmètre v1 arrêté (connexion, historique de commandes, édition de ses propres infos — pas de liste de souhaits/moyens de paiement enregistrés/fidélité pour l'instant) et mécanisme d'authentification choisi (lien magique par email, pas de mot de passe) avec la justification complète consignée dans la conversation plutôt que redonnée ici en détail.

**Nouveau module `compte-client`** (`modules/compte-client/`), suit le pattern standard (`module.meta.json`, autorisation agence + activation tenant, route toujours mappée/404 si désactivé) :

- **`CustomerLoginToken.cs`** (nouvelle table, migration `AddCustomerLoginTokens`) : lien de connexion à usage unique, 15 minutes de validité. Volontairement en base (pas un token signé sans état comme les autres) : la validation en 2 temps (GET affiche "confirmer ?", **seul un POST derrière un vrai clic** marque le lien utilisé et ouvre la session) a besoin d'un état serveur pour empêcher le rejeu — sinon un scanner de sécurité de messagerie (Outlook Safe Links, Proofpoint...) qui pré-visite automatiquement les liens d'un email grillerait le lien avant même que le client ne l'ouvre. Piège connu des liens magiques, traité dès la conception plutôt que découvert après coup.
- **`CustomerToken.cs`** : session signée (HMAC-SHA256), dupliqué depuis `AdminToken.cs` plutôt que réutilisé — payload différent (`CustomerId`+`ClientSiteId`, pas de notion de scope agence/tenant) et durée de vie bien plus longue (60 jours contre 1h : un client revient rarement, pas question de lui redemander un lien à chaque visite). Réutilise la même clé de signature (`Admin:TokenSecret`) que les tokens admin — un secret HMAC applicatif générique, pas littéralement scopé aux privilèges admin.
- **`CompteClientModule.cs`** : `POST /account/request-login` (toujours la même réponse générique, qu'un client existe ou non — jamais de fuite d'existence), `GET /account/verify-login` (lecture seule), `POST /account/confirm-login` (marque le lien utilisé, émet la session), `GET`/`PUT /account/me` (profil + commandes propres, email non modifiable en self-service — reste à la main du tenant). Un compte n'existe que pour un email qui a déjà une fiche `Customer` (première commande, ou ajout manuel par le tenant) — pas de création de compte "à vide" en v1.
- **`BrevoEmailService.SendCustomerLoginLinkAsync`** : même gabarit visuel que les autres emails transactionnels, CTA vers le lien de connexion.
- **Frontend** : `AccountPage.tsx`, page standalone partagée par les 3 templates (même principe que `CartPage.tsx` — bandeau nom/logo + footer aux couleurs du template actif, contenu neutre). Gère les 3 états (formulaire de connexion / confirmation du lien / tableau de bord) dans un seul composant selon la présence de `?token=` et d'une session en `localStorage` (`etnof-customer-session-{clientSiteId}`). Lien vers `/compte` ajouté à la nav des 3 templates (icône dédiée dans le header de Charis, à côté du panier ; lien texte pour Hestia/Helios, qui n'ont pas ce header à icônes).

Testé de bout en bout (curl pour le backend, CDP/Chrome headless piloté sur la vraie UI pour le frontend) sur Atelier Lumen, module autorisé+activé pour l'occasion (laissé actif à la fin de la session, comme pour Horaires en juillet, pour qu'Ethan puisse l'essayer directement) : demande de lien → ligne `CustomerLoginToken` créée + email accepté par Brevo (201) ; page de confirmation affiche le bon nom ; clic sur "Confirmer" → session ouverte, redirection propre (`?token=` retiré de l'URL) ; tableau de bord affiche la bonne commande (Trench Long Beige, 165€) ; édition du profil → sauvegarde confirmée ; token réutilisé une seconde fois → rejeté (usage unique fonctionnel) ; token émis pour Atelier Lumen testé sur un autre tenant → rejeté (isolation multi-tenant) ; accès sans token → 401 ; déconnexion → session effacée, retour à l'écran de connexion ; rechargement de page avec session déjà en `localStorage` → reste connecté. Toutes les données de test (nom modifié, tokens de connexion) nettoyées après vérification. `dotnet build -c Release` et `tsc -b` propres, backend redémarré pour charger le nouveau code.

**Reste à faire, hors scope v1 assumé** : liste de souhaits (sujet séparé), moyens de paiement enregistrés, programme de fidélité, création de compte sans commande préalable. **Reste à confirmer par Ethan** : essayer le flux complet lui-même (recevoir un vrai email et cliquer le vrai lien, pas seulement vérifié via requêtes directes au token en base) sur Atelier Lumen.

---

## Session du 2026-08-31 — module Offres, pages légales, multi-comptes admin, historique des commandes, SEO avancé

Grosse session (une douzaine de sujets traités à la suite, chacun parti d'un retour concret d'Ethan). Détail par sujet ci-dessous, testé au fil de l'eau (`tsc -b`/`dotnet build -c Release` propres à chaque étape) mais **rien vérifié visuellement dans le vrai navigateur par Claude Code** (pas d'outil de capture d'écran dans cet environnement, contrairement aux sessions précédentes en CDP/Chrome headless) — tout reste **à confirmer par Ethan**, il a suivi/validé chaque étape via ses propres captures d'écran au fil de la session.

**Module Offres + mécanisme "Gratuit"** : la page "Offres" (grille tarifaire indépendante du Catalogue) devient un vrai module (`modules/offres/module.meta.json`, gate admin + affichage public Hestia/Helios), suivant le même pattern que Horaires (donnée toujours dans `SiteContent.Offers`, pas de migration). `ModulePrice.Price` accepte maintenant la sentinelle `"Gratuit"` (case à cocher dans `PricingSection.tsx`, agence) — l'autorisation par tenant reste 100% manuelle (décision explicite d'Ethan : un module gratuit n'est pas activé automatiquement pour tout le monde). Maps et Horaires basculés sur cette sentinelle au passage (Maps avait en fait 100€ en base, incohérent avec la doc — corrigé).

**Mentions légales / Politique de confidentialité — core, pas un module** : même traitement que les CGV (`SiteContent.LegalNoticeContent`/`PrivacyPolicyContent`, migration `AddSiteContentLegalPages`, onglet "Mentions légales" dans `EstablishmentSection.tsx`, pages publiques `/mentions-legales`/`/confidentialite`). Nouvelle barre de liens légaux (CGV/Mentions légales/Confidentialité, seulement les pages renseignées) rendue par `SiteFooter.tsx` **sous** le footer plutôt que fondue dedans (demande explicite d'Ethan) — exportée (`LegalLinksBar`) et réutilisée directement sous le bloc "Mes commandes" de la page Compte client (mieux placée qu'en bas de page, trop loin du regard).

**Paiement bloqué légalement, côté serveur** : `StripeModule.cs` (`POST /stripe/checkout`) refuse désormais de créer une session Stripe si les CGV **ou** la Politique de confidentialité sont vides — avant, seul `CartPage.tsx` cachait le bouton (contournable en appelant l'API directement). Nouveau statut `ClientSite.Status` **"Prospection"** pour un site de démo/test sans vraie vente à protéger, qui échappe à ce garde-fou ; un site créé normalement reste par défaut sur "En cours", jamais "Prospection" par accident.

**`SaveButton` (composant partagé)** : remplace le pattern répété "bouton + `<span>Enregistré</span>` séparé" par un seul composant (`frontend/src/components/admin/SaveButton.tsx`) qui porte lui-même le spinner de chargement, la couleur succès/erreur, et le retour automatique à l'état normal après 3s (états "saved"/"error" verrouillés, non cliquables pendant ce délai). Appliqué à toutes les pages admin d'un tenant, à l'espace agence (Entreprise, Paiement), et en variante compacte (`size="sm"`) aux boutons "Enregistrer" par ligne (Tarifs des modules, Multilingue, Avis Google — qui gagnent au passage un vrai état d'erreur, absent avant).

**Page "Mon compte" (module Compte client) retravaillée** : labels au-dessus des champs (avant : seulement des placeholders, illisible une fois rempli), adresse sur sa propre ligne pleine largeur, formulaire en **lecture seule par défaut** avec bouton "Modifier mes informations" (avant : formulaire toujours ouvert, incitait à modifier des infos qui n'ont pas besoin de l'être). Photo produit dans l'historique de commandes (`OrderItem.ImagePath`, copié depuis la 1ʳᵉ photo du produit au moment de la commande, migration `AddOrderItemImagePath`) et bouton "Contacter l'établissement" (mailto vers `SiteContent.Email`, jamais `ManagerEmail`).

**Multi-comptes admin (rôle "Employé")** : nouvelle table `TenantAdminAccount` (Prénom/Nom/Email/Téléphone) — le Propriétaire reste `ClientSite.PasswordHash`, jamais dupliqué. Connexion quotidienne par email + mot de passe (pas de lien magique à chaque fois, décision d'Ethan pour la simplicité au quotidien), mais le mot de passe lui-même est **défini par l'employé**, jamais choisi/transmis par le Propriétaire : création d'un compte envoie un email d'invitation (`TenantAdminAccountInvite`, 48h, usage unique, même patron 2-temps que `CustomerLoginToken`) vers une page dédiée (`TenantInvitationPage.tsx`, `/admin/{clientSiteId}/invitation`) où l'employé choisit son mot de passe et se retrouve connecté directement. Accès restreint côté serveur (`TenantAdminAuth.IsOwnerAuthorizedAsync`) : pas de Modules, Paiement Stripe, ni la page "Comptes" elle-même — tout le reste (Établissement, Produits, Commandes, Blog...) reste accessible. `AdminToken` porte maintenant un `AccountId` optionnel pour distinguer précisément quel employé agit.

**Session admin** : renouvellement automatique tant que l'utilisateur est actif (`POST /api/admin/refresh-token`, appelé au plus 1x/5min sur une vraie activité clic/frappe/scroll) plutôt qu'une expiration fixe à 1h même en pleine utilisation — resté inactif, le token expire normalement. Bouton "Se déconnecter" ajouté (absent jusqu'ici) dans les deux sidebars (tenant et agence). Champs de connexion dotés des bons attributs `autocomplete`/`name` + rechargement complet après connexion réussie, pour que le gestionnaire de mots de passe du navigateur propose réellement d'enregistrer (repli AJAX pur, très peu fiable pour ça).

**Historique des actions (`AdminActionLog`)** : capture générique (middleware, `Program.cs`) de toute requête d'écriture réussie sous `/admin/...` d'un tenant, quel que soit l'endpoint — pas d'ajout à la main requis par future page admin. Connexions aussi loguées (Propriétaire, Employé nommé, **Agence via le mot de passe passe-partout, jamais masqué** — décision d'Ethan, la transparence protège le client autant que l'agence). Page "Historique" côté tenant (owner-only) et modale côté agence (`SitesSection.tsx`), pagination par 20 ("Charger plus").

**Suivi + commentaires par commande** : `OrderStatusChange` (timeline des changements de statut avec date + auteur, une ligne uniquement si le statut change vraiment) et `OrderComment` (notes internes libres, jamais visibles du client) affichés dans la ligne dépliée de `OrdersSection.tsx` (`OrderDetailPanel.tsx`, nouveau).

**SEO avancé** : lien `canonical` + JSON-LD (`LocalBusiness` sur la home, `Product` sur la fiche produit Charis, `BlogPosting` sur un article — `frontend/src/utils/structuredData.ts`, pas importable depuis un module par isolation, voir `BlogPostPage.tsx` qui duplique une version minimale) et `sitemap.xml`/`robots.txt` par tenant (`backend/SeoEndpoints.cs`, servis sous `/api/t/{clientSiteId}/...` — un vrai domaine par client nécessitera un rewrite côté hébergement, hors du code du socle).

**Divers** : favicon par défaut du socle (`frontend/public/favicon.svg`, navy/vert) affiché tant qu'un tenant n'a pas son propre logo — avant, onglet vide. Sous-menu de la sidebar réduit à un lien simple quand un seul enfant reste visible (ex. "Site" sans le module Offres). KPI "Offres" du tableau de bord remplacé par "Produits" quand ce module n'est pas actif. Chat IA et FAQ IA marqués **abandonnés** (pas juste dépriorisés). Images générées par Ethan pour les modules Offres et Compte client (`frontend/public/module-icons/`, ce dernier manquait même son entrée dans le registre).

**Reste à confirmer par Ethan** (rien vérifié visuellement côté Claude Code cette session) : à peu près tout ce qui précède, en particulier le flux d'invitation employé de bout en bout (recevoir le vrai email, définir un mot de passe, vérifier l'accès restreint), le blocage serveur du paiement sur un site pas en "Prospection", et le rendu de `sitemap.xml`/`robots.txt`.
